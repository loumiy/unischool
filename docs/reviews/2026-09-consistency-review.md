...
4. **The catalog's placeholders filled wrong** in ways every player sees: "the the class of 7", "The Men's Soccer Team team", "Science has appeared in a guidebook", "A section of roof has come off Campus Quad" (Plan 45).
...
| Demolition took back beds only for dorms; the Graduate College's 600 stayed. A disbanded housed chapter kept its house's beds. | Any kind's `capacityBonus` comes off (and a dorm's stories); a dissolved housed chapter's beds come off. | `cb9477a` |
...
| The housing drawer left out stories and project beds, so it did not add up to capacity. | It counts every standing building's beds and a dorm's stories; checked equal to capacity on five saves. | `a1cbba1` |
...
| The text-size setting missed 22 fixed pixel sizes; the color-vision setting missed the alert badge and the map's refusal tooltip, and a help line said "a red bar". | Both reach them. | `6202ca8` |
| A picked promise looked unpicked; two color tokens were undefined; about 35 classes were dead (51 rules). | Styled; replaced; removed. | `f83d1a9`, `dd8943a` |
| The build menu collapsed tiles under an unnamed group; the faculty header clipped "USED/HAVE"; a coach's age was a bare number; the research card's Wind up wrapped. | Named, widened, labeled, kept top right. | `35289f7`, `d9ca779`, `a426f0b` |
| Unlabelled inputs (the name field, Curriculum search and filter); radio semantics on the seat policies; the funds button labeled "Open Treasury" when open; the Final Report's mark hint twice. | Labeled, corrected, once. | `6202ca8`, `bff1578` |
...
- Law had no research interests, so every Law bio read "research centers on the field".
...
- **Spelling:** American throughout, in text, docs and comments (the owner's call). Identifiers and save field names keep their spelling.
- **Quotes:** straight ASCII quotes.

Plan 43 carries the same rules into the two files it owned (`f51a440`, `937d7ab`, `351c953`).

**Open ([Q14](#q14)):**
- choices labeled as paid that cost nothing;
...
- **Drawing data** (data review, section 4).
  - Stories were counted twice: a library's first new floor took it from 3 stories to 5.
  - Chapter houses fell through to default footprint, motif and height.
  - Capital projects stood at two stories in the default wall.
...
<a id="q3"></a>**Q3. The construction freeze** (the distress ladder's "no new construction") stops new builds but not added stories, venue expansions or library floors. *Recommend:* freeze those too; they are construction.
...
<a id="q10"></a>**Q10. Authored text in a save is frozen.** Buildable names and descriptions live in the save, so a data correction (this review's included) reaches new runs only. *Recommend:* refresh the static text from the catalog on load, leaving run-generated names alone.
...
- Cut the catalog copies of the recruiting scandal, the coach poached and naming rights.
...
<a id="q16"></a>**Q16. The harness's strategies hang on the college's name.** Program offers are seeded from the name (`programOffers.ts`). Midway through this review a fix changed the harness's founding name from "Test University" to "Test". That alone took the Balanced builder from 13,280 students in Year 12 to 1,348, with 4 milestones instead of 19. The fix was moved so the harness's name is unchanged. But a strategy that collapses on a different draw of offers is measuring one seed's luck with its program list, not the game. *Recommend:* run the scorecard over a few founding names as well as seeds, and make the strategies take whatever offer fits.
