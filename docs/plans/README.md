...
| 22 | [Foundations](22-foundations.md) | Phase A of the merge with UniSchool v2: a test runner and CI, one formatter, dead code out, a seeded random stream in the state, saving out of the reducer, an action log and replay, the comment trim, content integrity checks, the merged harness and v2's tools. PRs A–J. | In progress |
| 23 | [The ladder](23-the-ladder.md) | Phase B of the merge: the unlock track. Named milestones, each opening buildings and tabs on one condition, shown as a ladder with progress and a letter as each lands; the scattered unlock gates and tab gates fold into it. PRs A–E. | In progress |
