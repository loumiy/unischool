...
| `civic` | 2.9 × 3.2 | 0.91 | student centre, dining, clinic, arena |
...
Four of the six sit between 0.62 and 0.91 — one family of proportions, which is
...
| **G2** ✅ | The hospital drawn from a reference photograph: a tall ward slab with a lower glazed public wing in front, a curtain wall, a recessed entrance level, a red cross, and a material of its own | Asked for after G. **Shipped:** the `block` motif's large instances split into two wings; the small one (the computing centre, 4x3) stays a single box. |
