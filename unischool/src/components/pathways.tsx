...
// turn reads as a curve through the tile centres rather than as steps. The
...
//
// The kerb is the fill's own outline, stroked under the fill: where paving
...
export function buildPathGeometry(pathways: Pathways): Geometry {
  ...
  for (const { row, col } of tiles) {
    ...
    // A bridge to the diagonal neighbour j: the two tiles meet only at a
    // corner.
    ...
    corners.forEach(([cc, cr, ic, ir], k) => {
      ...
      // An arc round the corner's inset centre, from the edge before it to
